...
const STORAGE_KEY = "openrc.demo.scenario.v1";
...
export function AtcSimulator() {
  ...
  const [headingDraft, setHeadingDraft] = useState("090");
  const [altitudeDraft, setAltitudeDraft] = useState("7000");
  const [speedDraft, setSpeedDraft] = useState("240");
  const [waypointNameDraft, setWaypointNameDraft] = useState("");
  const [notice, setNotice] = useState("Simulation initialized. Right-click the scope to create a custom waypoint.");
  ...
  useEffect(() => {
    ...
    setHeadingDraft(String(Math.round(selectedAircraft.assignedHeadingDeg || 360)).padStart(3, "0"));
    setAltitudeDraft(String(Math.round(selectedAircraft.assignedAltitudeFt)));
    setSpeedDraft(String(Math.round(selectedAircraft.assignedSpeedKts)));
  }, [selectedAircraft?.id, selectedAircraft?.assignedHeadingDeg, selectedAircraft?.assignedAltitudeFt, selectedAircraft?.assignedSpeedKts]);
  ...
  const applyCommands = () => {
    if (!selectedAircraft) return;
    engine.issueHeading(selectedAircraft.id, Number(headingDraft));
    engine.issueAltitude(selectedAircraft.id, Number(altitudeDraft));
    engine.issueSpeed(selectedAircraft.id, Number(speedDraft));
    refresh();
  };
  ...
  return (
    <main className="sim-shell">
      <header className="topbar">
        <div className="brand">
          ...
          <div>
            ...
            <span>Open Radar Control · vertical slice 0.1</span>
          </div>
        </div>
        ...
        <div className="top-actions">
          ...
          <button onClick={() => { engine.reset(); setSelectedAircraftId("fdx1205"); setSelectedWaypointId(null); setNotice("Demo scenario reset to seed 938224."); refresh(); }}>Reset</button>
        </div>
      </header>
      ...
      <section className="status-ribbon">
        ...
        <span>{state.positions.find((position) => position.id === state.humanPositionId)?.frequency} MHz</span>
        ...
        <span className="demo-warning">DEMO NAV DATA · NOT FOR NAVIGATION</span>
      </section>
      ...
      <div className="workspace">
        <section className="scope-panel">
          ...
          <div className="scope-toolbar">
            ...
            <div className="scope-hint">Right-click empty scope → Create Waypoint · Left-click target/waypoint → inspect</div>
          </div>
        </section>
          ...
          <section className="rail-card strips-card">
            ...
            <div className="strip-list">
              {visibleAircraft.map((aircraft) => {
                ...
                return (
                  <button key={aircraft.id} className={`flight-strip ${selectedAircraftId === aircraft.id ? "selected" : ""} ${incoming ? "incoming" : ""} ${alert ? "alert" : ""}`} onClick={() => { setSelectedAircraftId(aircraft.id); setSelectedWaypointId(null); }}>
            ...
            {selectedAircraft ? (
              <>
                ...
                <div className="command-grid">
                  <label>HDG<input value={headingDraft} onChange={(event) => setHeadingDraft(event.target.value.replace(/\D/g, "").slice(0, 3))} /></label>
                  <label>ALT FT<input value={altitudeDraft} onChange={(event) => setAltitudeDraft(event.target.value.replace(/\D/g, "").slice(0, 5))} /></label>
                  <label>SPD KT<input value={speedDraft} onChange={(event) => setSpeedDraft(event.target.value.replace(/\D/g, "").slice(0, 3))} /></label>
                </div>
                <div className="command-actions">
                  <button className="primary" disabled={selectedAircraft.controllerPositionId !== state.humanPositionId} onClick={applyCommands}>Issue clearance</button>
                  <button disabled={selectedAircraft.controllerPositionId !== state.humanPositionId} onClick={() => { engine.resumeRoute(selectedAircraft.id); refresh(); }}>Resume route</button>
                  <button disabled={selectedAircraft.controllerPositionId !== state.humanPositionId} onClick={() => { engine.requestNextHandoff(selectedAircraft.id); refresh(); }}>Handoff next</button>
                </div>
                <div className="route-readout">
                  <span>ROUTE / {selectedAircraft.navigationMode}</span>
                  <strong>{selectedAircraft.flightPlan.route.join("  ›  ") || "—"}</strong>
                  <small>{selectedAircraft.flightPlan.procedureId ?? "NO PUBLISHED PROCEDURE"}</small>
                </div>
                ...
                {selectedAircraft.controllerPositionId !== state.humanPositionId && !incomingHandoffs.some((handoff) => handoff.aircraftId === selectedAircraft.id) && <p className="ownership-note">Owned by {selectedAircraft.controllerPositionId}. You can observe it, but AI is issuing its clearances.</p>}
              </>
            ) : <p className="empty-state">Select an aircraft on the scope or from a strip.</p>}
          </section>
        </aside>
      </div>
      ...
      <div className="lower-deck">
        ...
        <section className="lower-card waypoint-card">
          <div className="lower-heading"><span>CUSTOM WAYPOINT</span><strong>{selectedWaypoint?.source === "CUSTOM" ? selectedWaypoint.name : "NONE"}</strong></div>
          ...
          ) : <p className="quiet-state">Right-click an empty part of the radar and choose Create Waypoint. Generated names are unique five-letter identifiers.</p>}
        </section>
        ...
        <section className="lower-card procedure-card">
          ...
          <select value={selectedProcedure?.id ?? ""} onChange={(event) => setProcedureId(event.target.value)}>
            {state.procedures.map((procedure) => <option key={procedure.id} value={procedure.id}>{procedure.name} · {procedure.type}{procedure.immutable ? " · LOCKED" : ""}</option>)}
          </select>
          {selectedProcedure && (
            <>
      ...
      <footer className="notice-bar"><span>OPENRC</span><p>{notice}</p><b>$0 REQUIRED SERVICES · LOCAL SIMULATION CORE</b></footer>
    </main>
  );
}
